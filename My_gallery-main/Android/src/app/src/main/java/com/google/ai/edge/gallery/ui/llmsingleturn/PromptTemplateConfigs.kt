...
enum class PromptTemplateType(
  ...
  SURVIVAL_ADVICE(
    label = "Survival advice (Bear Grylls)",
    config = PromptTemplateConfig(), // No input editors needed for now
    genFullPrompt = { userInput, _ ->
      buildAnnotatedString {
        withStyle(GEMINI_GRADIENT_STYLE) {
          append(
            """
    ...
    examplePrompts = listOf(
      "I'm lost in a snowy mountain with no food. What do I do?",
      "I fell into a river and my clothes are soaked. How can I stay warm?",
      "How do I purify water in the jungle?",
