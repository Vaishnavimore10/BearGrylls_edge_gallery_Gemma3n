...
import com.google.ai.edge.gallery.data.SurvivalKnowledgeRetriever
...
open class LlmChatViewModelBase(
  val curTask: Task,
  private val application: Application // Injected via Hilt or manually
) : ChatViewModel(task = curTask) {

  // Optional: knowledge retriever, can be used later
  private val knowledgeRetriever = SurvivalKnowledgeRetriever(application)
  init {
    // NEW: Launch a coroutine to call the suspend function in the background.
    // This will NOT block the main thread, and your app will not crash.
    viewModelScope.launch {
      knowledgeRetriever.initialize()
    }
  }
...
) : LlmChatViewModelBase( curTask = TASK_LLM_CHAT,application,)

@HiltViewModel
class LlmAskImageViewModel @Inject constructor(
  application: Application
) : LlmChatViewModelBase( curTask = TASK_LLM_ASK_IMAGE,application)

@HiltViewModel
class LlmAskAudioViewModel @Inject constructor(
  application: Application
) : LlmChatViewModelBase( curTask = TASK_LLM_ASK_AUDIO,application)